...
inst.addEventListener("keyup", () => {
    if (inst.value.length > 250) {
        alert("Max 250 characters allowed!");
        inst.value = "";
...
document.forms[0].addEventListener('click', () => {
    document.getElementById("autocomplete__widget").style.display = "none";
})
...
const submitStep4 = () => {
    ...
    const detailObj = {
        ...
        jobReference: localStorage.getItem("job") === null ? document.getElementById("job").value : localStorage.getItem("job"),
        driverInstructions: localStorage.getItem("inst") === null ? document.getElementById("inst").value : localStorage.getItem("inst"),
    ...
    if (img.dataset.booking === "quote") {
        ...
        fetch("/quote/step-4/" + session_id, {
            ...
            console.log(data);
            window.location.href = "/quote/checkout" + "/" + session_id + "?t=2";
        }).catch((err) => {
            console.log(err);
    ...
    fetch("/booking/step-4", {
        ...
        window.location.href = "/booking/checkout?q=" + data.id;
        return;
    })
}